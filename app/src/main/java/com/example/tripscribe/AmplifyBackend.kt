...
object AmplifyBackend {
    private val TAG = AmplifyBackend::class.java.simpleName

    fun initialize(applicationContext: Context): AmplifyBackend {
        try {
        ...
        Amplify.Auth.fetchAuthSession({ result ->
            ...
            when (cognitoAuthSession.identityId.type) {
                AuthSessionResult.Type.SUCCESS -> Log.i(
                    TAG, "IdentityId: " + cognitoAuthSession.identityId.value
                )
    ...
    fun signOut() {
        ...
        Amplify.Auth.signOut({ Log.i(TAG, "Signed out!") },
